...
@export_range(1, 16, 1) var hushling_count: int = 6
...
func _spawn_hushlings(creature_root: Node3D) -> void:
	...
	var count: int = hushling_count if randomize_spawn_positions else hushling_offsets.size()
...
func _sample_spawn_position() -> Vector3:
	...
	for attempt in range(32):
		var local_offset := Vector3(
			_rng.randf_range(-half_extents.x, half_extents.x),
			_rng.randf_range(-half_extents.y, half_extents.y),
			_rng.randf_range(-half_extents.z, half_extents.z)
		)
	...
	var fallback_position: Vector3 = _offset_to_world(habitat_center_offset)
	_spawned_world_positions.append(fallback_position)
